...
const validateQuery = (args) => {
    ...
    const disctrict = args.disctrict ? [{ '$match': { disctrict: args.disctrict } }] : []

    const filterNameSupplier = args.name_supplier ? [{ $eq: ["$name", args.name_supplier] }] : []
...
module.exports = async (args) => {
    ...
    const total = await Cinema.aggregate([...aggregate, { $count: 'number' }]).then(res => res[0] ? res[0].number : 0)
